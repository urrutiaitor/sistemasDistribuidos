	...
	protected static void attendClient(Socket sock) {
		...
		try {
			...
			while ((operation = sPrx.waitCommand(operandos)) != ServerProxy.connectionClosed) {
				switch (operation) {
				case 2:
					System.out.println("Doing GCD of " + operandos[0] + " and " + operandos[1]);
					result = servant.gcd(operandos[0], operandos[1]);
					System.out.println("Answer of GCD: " + result);
					sPrx.answer(result);
					break;
				case 1:
					System.out.println("Doing factorial of " + operandos[0]);
					result = servant.factorial(operandos[0]);
					System.out.println("Answer of factorial: " + result);
					sPrx.answer(result);
					break;
				default:
					sPrx.answerNull();
